...
static bool split_if_too_big( struct block_header* block, size_t query ){
    ...
    if(block && block_splittable(block, check_query)){
...
struct block_search_result block_search_result_create(struct block_header* restrict block, enum search_result type){
    return (struct block_search_result){.block = block, .type = type};
}

static struct block_search_result find_good_or_last  ( struct block_header* restrict block, size_t sz )    {
    if(!block){
        return block_search_result_create(NULL, BSR_CORRUPTED);
    }
    while(block){
        while (try_merge_with_next(block));
        if (block_is_big_enough(sz, block) && block->is_free) {
            split_if_too_big(block, sz);
            return block_search_result_create(block, BSR_FOUND_GOOD_BLOCK);
        }
        if(!block->next) break;
        block = block->next;
    }
    return block_search_result_create(block, BSR_REACHED_END_NOT_FOUND);
    ///
}
...
static struct block_header* grow_heap( struct block_header* restrict last, size_t query ) {
    if(last){
        ...
        if(region_is_invalid(new_region.addr)) return NULL;
